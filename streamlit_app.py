...
# ==============================================================================
# KELAS LOGIKA BISNIS (DENGAN KONEKSI DATABASE PERMANEN)
# ==============================================================================
...
class SugarTracker:
    def __init__(self):
        ...
        # --- PERUBAHAN 1: Inisialisasi Koneksi Database ---
        # Menggunakan koneksi SQL bawaan Streamlit untuk penyimpanan permanen
        self.conn = st.connection("history_db", type="sql")
        # Buat tabel 'history' jika belum ada saat aplikasi pertama kali dijalankan
        ...
        # Muat riwayat dari database saat aplikasi dimulai
        self.history = self._load_history()

    # --- PERUBAHAN 2: Memuat Riwayat dari Database ---
    def _load_history(self) -> Dict:
        """Memuat riwayat konsumsi dari database SQL."""
        with self.conn.session as s:
            result = s.execute("SELECT date, data FROM history;").fetchall()
            # Ubah hasil query (list of tuples) menjadi format dictionary
            history_data = {row.date: json.loads(row.data) for row in result}
        return history_data

    # --- PERUBAHAN 3: Menyimpan Riwayat ke Database ---
    def _save_history(self):
        """Menyimpan data riwayat ke database SQL."""
        with self.conn.session as s:
            for date_str, data_dict in self.history.items():
                # Gunakan "UPSERT": INSERT baru jika belum ada, atau UPDATE jika tanggal sudah ada
                s.execute(
                    """
                    INSERT INTO history (date, data) VALUES (:date, :data)
                    ON CONFLICT(date) DO UPDATE SET data = excluded.data;
                    """,
                    params=dict(date=date_str, data=json.dumps(data_dict))
                )
            s.commit()
    ...
    def archive_and_reset_day(self):
        ...
        if total_gula > 0 and self.user_profile:
            ...
            # Update dictionary history di memori terlebih dahulu
            self.history[today_str] = {
                'total_gula': round(total_gula, 2),
                'limit_kemenkes': limits.get('kemenkes'),
                'limit_aha': aha_limit_value
            }
            # Simpan seluruh state history ke database
            self._save_history()
            
        self.reset_daily_intake()

    # --- Database makanan lengkap (tidak ada perubahan di sini) ---
    ...
    # ... Sisa method lainnya (set_user_profile, add_food_item, dll.) tetap sama ...
...
if menu == "Laporan Harian":
    ...
    if profile:
        limits = st.session_state.tracker.get_recommended_limit()
        st.success(f"Batas konsumsi gulamu: **Kemenkes: {limits['kemenkes']}g**, **AHA: {limits['aha']}g** per hari.")import streamlit as st
import json
from datetime import datetime, date, timedelta
from typing import Dict, List, Any
import pandas as pd
